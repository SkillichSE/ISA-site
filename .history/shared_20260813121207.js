...
(function initIsaCard() {
  ...
  async function isaCard(withLogo) {
    ...
    // border frame - removed rounded corners

    // Draw logo if present
    if (logo) {
      const logoSize = 180;
      ctx.drawImage(logo, 32, 20, logoSize, logoSize);
    }
    ...
    // Tag dimensions - match CSS: 0.75rem font, 4px 10px padding
    const tagH = 20, tagPadX = 10, tagGapX = 6, tagGapY = 8;

    // Calculate how many rows of tags we need
    ...
    tags.forEach((tag) => {
      ...
      if (currentRowWidth + w + (currentRow.length > 0 ? tagGapX : 0) > W - PAD * 2 && currentRow.length > 0) {
        // Start new row
        tagsRows.push(currentRow);
        currentRow = [tag];
        currentRowWidth = w;
      } else {
        currentRow.push(tag);
        currentRowWidth += w + (currentRow.length > 1 ? tagGapX : 0);
      }
    });
    if (currentRow.length > 0) tagsRows.push(currentRow);
    
    const totalTagsHeight = tagsRows.length * tagH + (tagsRows.length - 1) * tagGapY;
    ...
    const tagsTop = urlTop - totalTagsHeight - 8;
