...
const { rehostImage } = require('./supabase-storage');
...
async function syncChannel(env, token, channel) {
  ...
  if (!isFirstRun) {
    for (const message of ordered) {
      ...
      const cleanText = convertDiscordTimestamps(stripMentions(rawText));
      if (!cleanText) continue;

      // Discord's attachment URLs are signed and expire, so pull the image
      // into Supabase Storage now rather than linking the site straight to
      // Discord. If the re-host fails for some reason, fall back to the
      // original link so the news item still publishes with an image today.
      let imageUrl = image.url;
      try {
        imageUrl = await rehostImage(env, image.url);
      } catch (err) {
        console.error(`discord-news-sync: failed to rehost image for message ${message.id}:`, err.message);
      }
